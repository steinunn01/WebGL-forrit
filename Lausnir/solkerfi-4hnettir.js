...
//     Tunglið snýst í kringum jörðina og gervihnöttur (LRO)
//     snýst um tunglið.  Hægt að breyta hraða hermunar með
//     upp/niður örvum.
//
...
var simSpeed = 1.0;     // Hraði hermunar
...
function render()
{
    ...
    rotDay += 1.0*simSpeed;
    rotYear += 0.1*simSpeed;
	rotLRO += 10.0*simSpeed;
    ...
    // teikna gervihnött um tunglið
    mvt = mult( mvt, rotateX( rotLRO ) );
    mvt = mult( mvt, translate( 0.0, 2.0, 0.0 ) );
    mvt = mult( mvt, scalem( 0.4, 0.4, 0.4 ) );
    gl.uniformMatrix4fv(matrixLoc, false, flatten(mvt));
    gl.drawArrays( gl.TRIANGLES, 0, numVertices );
